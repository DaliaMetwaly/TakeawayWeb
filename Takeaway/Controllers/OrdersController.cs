...
namespace Takeaway.Controllers
{
    ...
    public class OrdersController : Controller
    {
        ...
        public ActionResult LoadData()
        {
            ...
            //    ? db.Orders.Where(a => a.IsDetete == false  && a.RestaurantData.Restaurant.UserID == usrID && a.OrderStatu.IsDone==true).ToList().Select(i => new
            //    {
            //        Id = i.ID,
            ...
            //    }).ToList()
            //    : db.Orders.Where(a => a.IsDetete == false && a.OrderStatu.IsDone == true).ToList().Select(i => new
            //    {
            //        Id = i.ID,
            //        ContactName = i.User.ContactName,
            //        RestaurantName = i.RestaurantData.Restaurant.RestaurantName + " - " + i.RestaurantData.Region.RegionAr,
       ...
       public PartialViewResult DisplayOrderDetails(int ID)
        ...
        public JsonResult edittempDrinks(int Id)
        {
            ...
            var FoodItms = db.OrderDetails.Where(c => c.OrderID == Id).Select(c => new FoodEditTempList//&&c.ItemFoodParentID==0
            {
                ...
                ItemfoodId = c.ItemFoodID,
                OrderId = c.OrderID,
                CookingCatID = c.CookingCatID.Value == null ? 0 : c.CookingCatID.Value,
        ...
        public ActionResult Edit(int? id)
        {
            ...
            if (roleName == "Admin" || roleName == "SuperAdmin")
            {
                ViewBag.RestaurantID = new SelectList(db.Restaurants, "ID", "RestaurantName", order.RestaurantData.RestaurantID);
            }
            else if (roleName == "Restaurant")
            {
                ViewBag.RestaurantID = new SelectList(db.Restaurants.Where(x => x.UserID == usrID), "ID", "RestaurantName", order.RestaurantData.RestaurantID);
            }
        ...
        [HttpPost]
        public JsonResult Edit(OrderViewModel OrderViewModel)
        {
            List<OrderDetail> TempList = new List<OrderDetail>();
