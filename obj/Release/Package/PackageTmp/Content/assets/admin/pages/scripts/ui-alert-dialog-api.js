...
function ShowMsg(MessageText, MessageType, CloseTime) {
    Metronic.alert({
        ...
        type: MessageType,//$('#alert_type').val(),  // alert's type
        message: MessageText,//$('#alert_message').val(),  // alert's message
        close: true,//$('#alert_close').is(":checked"), // make alert closable
        reset: true,//$('#alert_reset').is(":checked"), // close all previouse alerts first
        focus: true,//$('#alert_focus').is(":checked"), // auto scroll to the alert after shown
        closeInSeconds: CloseTime,// $('#alert_close_in_seconds').val(), // auto close after defined seconds
        icon: 'check',//$('#alert_icon').val() // put icon before the message
    });
}
