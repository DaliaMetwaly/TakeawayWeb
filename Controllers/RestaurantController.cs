    ...
    public class RestaurantController : Controller
    {
        ...
        public ActionResult Create(string ResUserId,string ResUserName , int ?Msg)
        {
           ...
           //&& c.IsDetete == false), "ID", "RestaurantStatusAr");
            ...
            ViewBag.City = new SelectList(db.Cities.Where(c => c.IsActive == true
           && c.IsDelete == false), "ID", "CityAr");
        ...
        public ActionResult Create(RestaurantVM restaurantVM, HttpPostedFileBase FileName)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                restaurant.OneMachine = restaurantVM.OneMachine;
            ...
            if (ModelState.IsValid)
            {
             ...
             if (TempData["Address"] !=null)
             {
                 if ((TempData["Address"] as List<AddressList>).Count>0)
                 {
                        //DM 23-8-2017
                        //db.RestaurantDatas
                        //      .Where(x => x.ID == item.AddressID && x.Orders.Count <= 0)
                        //      .ToList()
                        ...
                        foreach (AddressList item in TempData["Address"] as List<AddressList>)
                    {
                            ...
                            if (item.AddressID == 0 && item.IsHidden == false)
                            {
                                //Add Restaurant Data
                                RestaurantData restaurantData = new RestaurantData();
                                restaurantData.Address = item.Address;
                            ...
                            //Check if exist Item need to delete
                            else if   (item.AddressID!=0 && item.IsHidden == true)
                            {
                                //Check if restaurant branch has orders
                                int orderCount = db.RestaurantDatas.Where(x => x.ID == item.AddressID).Select(x => x.Orders.Count).FirstOrDefault();

                                //If there is no orders add new record
                                if (orderCount<=0)
                                {
                                    //Delete Min Distance Price for this restauarnt branch
                                    db.MinDistancePrices.RemoveRange(db.MinDistancePrices.Where(x => x.RestaurantDataID == item.AddressID).ToList());
                                    //Set ISDeleted =true 
                                    RestaurantData restaurantDataItem =db.RestaurantDatas.Find(item.AddressID);
                                    restaurantDataItem.IsDetete = true;
                                }   
