    ...
    public class MinDistancePriceController : Controller
    {
        ...
        public ActionResult LoadData()
        {
            ...
            try
            {
                ...
                var Entity = (roleName == "Restaurant") ? db.MinDistancePrices.Where(x => x.IsDelete == false && x.Restaurant.UserID == usrID && x.IsActive == true).OrderByDescending(x => x.CreatedDate).ToList() : db.MinDistancePrices.Where(x => x.IsDelete == false  && x.IsActive==true).OrderByDescending(x => x.CreatedDate).ToList();
                ...
                Data = (from c in userData
                        ...
                        select new
                        {
                            ...
                            IsActive = db.MinDistancePrices.Where(x => x.IsActive == true).Any(v => v.RestaurantDataID == i.RestaurantDataID) ? "<span id='btn_Delete'  style='cursor:pointer;cursor:hand;' onclick='Change(" + i.RestaurantID + ")' class='glyphicon glyphicon-ok'></ span >" : "<span id='btn_Delete'  style='cursor:pointer;cursor:hand;' onclick='Change(" + i.RestaurantID + ")' class='glyphicon glyphicon-remove'></ span >",
                            ...
                            //IsActive = "<span id='btn_Delete'  style='cursor:pointer;cursor:hand;'  class='glyphicon glyphicon-ok'></ span >" ,
                            ...
                            Edit = "<a  id='btn_Edit' class='btn default btn-xs green' href='/MinDistancePrice/EditRestaurant?restaurantID=" + i.RestaurantID + "'>تعديل</a>",
        ...
        {
            var entity = db.MinDistancePrices.Where(i => i.IsDelete == false && i.IsActive==true && i.RestaurantID == id).ToList();
        ...
        public ActionResult Create(int? restaurantID)
        {
            ...
            List<int> RestauarantHasBranches = db.MinDistancePrices.Where(x => x.IsDelete == false && x.IsActive==true).Select(x => x.RestaurantID).Distinct().ToList<int>();
            ViewBag.RestaurantID = (roleName == "Restaurant") ? new SelectList(db.Restaurants.Where(x => x.RestaurantStatus == 1 && x.UserID == usrID && !RestauarantHasBranches.Contains(x.ID)), "ID", "RestaurantName") : new SelectList(db.Restaurants.Where(x =>x.RestaurantStatus == 1 && !RestauarantHasBranches.Contains(x.ID)), "ID", "RestaurantName");
            ...
            if (ModelState.IsValid)
            {
                ...
                if (TempData.Keys.Where(x => x.ToString() == "Distance").Count() > 0)
                {
                    foreach (DistanceList item in TempData["Distance"] as List<DistanceList>)
                    {
                        ...
                        minDistancePrice.IsActive = true;
        ...
        public ActionResult EditRestaurant(int? restaurantID)
        {
            ...
            ViewBag.RestaurantID = (roleName == "Restaurant") ? new SelectList(db.Restaurants.Where(x => x.RestaurantStatus == 1 && x.UserID == usrID && x.ID==restaurantID), "ID", "RestaurantName") : new SelectList(db.Restaurants.Where(x => x.RestaurantStatus == 1 && x.ID == restaurantID), "ID", "RestaurantName");
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult EditRestaurant(MinDistancePriceVM minDistancePriceVM)
        {
            
            ModelState.Remove("deliveryFeeValue");
            if (ModelState.IsValid)
            {
                List<MinDistancePrice> branchesToBeDeleted = db.MinDistancePrices.Where(x => x.RestaurantID == minDistancePriceVM.RestaurantID && x.IsDelete==false && x.IsActive==true).ToList();
                foreach(MinDistancePrice item in branchesToBeDeleted)
                {
                    item.IsDelete = true;
            ...
            ViewBag.RestaurantID = (roleName == "Restaurant") ? new SelectList(db.Restaurants.Where(x => x.RestaurantStatus == 1 && x.UserID == usrID && x.ID == minDistancePriceVM.RestaurantID), "ID", "RestaurantName") : new SelectList(db.Restaurants.Where(x => x.RestaurantStatus == 1 && x.ID == minDistancePriceVM.RestaurantID), "ID", "RestaurantName");
        ...
        public JsonResult GetRestaurantBranches(int restaurantID)
        {
            int Srno = 1;
            var Data = (Object)null;
            try
            {
                Data = db.MinDistancePrices.Where(x => x.IsDelete == false && x.IsActive==true && x.RestaurantID== restaurantID).ToList().Select(s => new
                {
                    count= Srno++,
                    Restaurant_id= s.RestaurantID,
                    Restaurant_Name=s.Restaurant.RestaurantName,
                    RestaurantData_id=s.RestaurantDataID,
                    Region_id =s.RegionID,
                    Region_Name=s.Region.RegionAr,
